...
- [x] **Cache↔downstream coupling documented & surfaced** — **FIXED 2026-05-28** — `main.nf` now prints a startup `log.info` showing the resolved `apptainer_cache_dir` plus an explicit reminder to set `apptainer.cacheDir` / `NXF_APPTAINER_CACHEDIR` to that same dir downstream; README has a new "Using the built images in your workflow" section documenting the coupling and the naming scheme. Still *unenforced* in code (no hard failure on mismatch — there's nothing to check against at build time), but no longer silent or undocumented.
